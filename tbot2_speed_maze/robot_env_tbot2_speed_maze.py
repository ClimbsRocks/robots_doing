...
class TurtleBot2RobotEnv(gazebo_env_generic.RobotGazeboEnv):
    ...
    # TODO: Genericize this.
        # Create some map, inside init, from sensor name to sensor topic, then just loop through that map, and check each one
        # And make sure we're doing something similar with our publishers
    ...
    def move_base(self, linear_speed, angular_speed, epsilon=0.05, update_rate=10):
        """
        Move the base based on the linear and angular speeds given.
        Wait untill those twists are achived (by? before?) reading from the odometry topic.
    ...
    def wait_until_twist_achieved(self, cmd_vel_value, epsilon, update_rate):
        """
        We wait for the robot to achieve the given cmd_vel Twist given by reading
        from the odometry.
